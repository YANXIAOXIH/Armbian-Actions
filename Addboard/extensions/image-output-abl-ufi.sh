    ...
    if [[ ! -x "/usr/local/bin/mkbootimg" ]]; then
        ...
        if curl -fsSL --connect-timeout 15 --max-time 30 --retry 2 -o /usr/local/bin/mkbootimg "${primary_url}" || \
           curl -fsSL --connect-timeout 15 --max-time 30 --retry 2 -o /usr/local/bin/mkbootimg "${backup_url}"; then
            ...
            exit_with_error "Failed to download mkbootimg from all sources. Build cannot proceed."
        fi
    fi
}
...
function post_build_image__900_convert_to_abl_img() {
    ...
    local fstype=$(blkid -s TYPE -o value "${ROOTFS_RAW}")
    ...
    if [[ "${fstype}" =~ ^ext[2-4]$ ]]; then
        ...
        display_alert "Sparse Image" "Btrfs optimization applied (skipping truncate)" "info"
    else
        display_alert "Sparse Image" "Unrecognized filesystem, attempting direct sparse conversion" "warning"
    fi
