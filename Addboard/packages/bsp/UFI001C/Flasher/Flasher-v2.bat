...
:: 环境与 UI 初始化
:: =============================================================================================
title UFI001C Armbian 智能刷机控制台 v2.5 (增强版)
...
echo %CYAN%=================================================================================================%RESET%
echo.
echo                       %GREEN%UFI001C Armbian 智能刷机控制台  v2.5%RESET%
echo.
echo %CYAN%=================================================================================================%RESET%
...
echo     [%CYAN% 5 %RESET%] 仅更新引导   %YELLOW%(选择并刷入 *lk1st.mbn 或 aboot.bin) - 测试功能%RESET%
...
set count=0
if exist "%FIRMWARE_PATH%\aboot.bin" (
    set /a count+=1
    set "aboot_file[!count!]=%FIRMWARE_PATH%\aboot.bin"
    set "aboot_name[!count!]=aboot.bin"
    echo   [%CYAN% !count! %RESET%] aboot.bin
)
...
if !count!==0 (
    echo %RED%[!] 错误: 未在 %FIRMWARE_PATH% 目录下找到任何 aboot.bin 或 *lk1st.mbn 文件！%RESET%
    echo.
    echo %GREEN%[?] 按任意键返回主菜单...%RESET% & pause >nul
    goto main_menu
)
...
if "!DO_BOOT!"=="0" goto skip_boot_select
...
:skip_boot_select
if "!DO_FIRMWARE!"=="0" goto confirm_execution

:select_firmware_aboot
cls
echo %CYAN%[*] 请选择全能刷机要使用的底层引导 (aboot) 文件:%RESET%
set fw_ab_count=0
if exist "%FIRMWARE_PATH%\aboot.bin" (
    set /a fw_ab_count+=1
    set "fw_aboot_file[!fw_ab_count!]=%FIRMWARE_PATH%\aboot.bin"
    set "fw_aboot_name[!fw_ab_count!]=aboot.bin"
    echo   [%CYAN% !fw_ab_count! %RESET%] aboot.bin ^(原厂默认^)
)
for %%F in ("%FIRMWARE_PATH%\*lk1st.mbn") do (
    set /a fw_ab_count+=1
    set "fw_aboot_file[!fw_ab_count!]=%%~fF"
    set "fw_aboot_name[!fw_ab_count!]=%%~nxF"
    echo   [%CYAN% !fw_ab_count! %RESET%] %%~nxF
)
if !fw_ab_count!==0 (
    echo %RED%[!] 错误: 未在 %FIRMWARE_PATH% 目录下找到任何 aboot.bin 或 *lk1st.mbn 文件！%RESET%
    echo.
...
echo.
set "fw_ab_idx="
set /p fw_ab_idx="%GREEN%[?] 请输入对应数字序号并按回车 [默认: 1]: %RESET%"
if "!fw_ab_idx!"=="" set "fw_ab_idx=1"

set "SELECTED_ABOOT_FILE=!fw_aboot_file[%fw_ab_idx%]!"
set "SELECTED_ABOOT_NAME=!fw_aboot_name[%fw_ab_idx%]!"

if "!SELECTED_ABOOT_FILE!"=="" (
    echo %RED%[!] 错误: 输入无效！%RESET%
    timeout /t 2 >nul
    goto select_firmware_aboot
)
...
echo %CYAN%===========================================================%RESET%
if "!DO_FIRMWARE!"=="1" echo   [%GREEN%OK%RESET%] 备份基带并刷写底层 ^(引导文件: %YELLOW%!SELECTED_ABOOT_NAME!%RESET%^)
...
if "!DO_FIRMWARE!"=="1" (
    ...
    "%JB%" flash aboot "!SELECTED_ABOOT_FILE!"
